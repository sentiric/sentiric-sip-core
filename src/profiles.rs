...
impl SipProfile for StandardProfile {
    ...
    fn add_response_headers(
        ...
        resp.headers
            .push(Header::new(HeaderName::UserAgent, self.user_agent()));
        resp.headers.push(Header::new(
            HeaderName::Allow,
            "INVITE, ACK, BYE, CANCEL, OPTIONS".to_string(),
        ));
        resp.headers.push(Header::new(
    ...
    fn determine_target(&self, _req: &SipPacket, src: SocketAddr) -> SocketAddr {
        src
    }
    fn should_send_trying_on_reinvite(&self) -> bool {
        true
    ...
    fn user_agent(&self) -> String {
        "Sentiric Media Gateway/1.0".to_string()
    }
}
...
impl SipProfile for LegacyProfile {
    ...
    fn add_response_headers(
        ...
        resp.headers
            .push(Header::new(HeaderName::UserAgent, "SentiricGW".to_string()));
        resp.headers.push(Header::new(
            HeaderName::Allow,
            "INVITE, ACK, BYE, CANCEL, OPTIONS".to_string(),
        ));
    ...
    fn user_agent(&self) -> String {
        "SentiricGW".to_string()
    }
}

fn extract_user_from_uri(uri: &str) -> String {
    let start = if let Some(idx) = uri.find("sip:") {
        idx + 4
    } else {
        0
    };
