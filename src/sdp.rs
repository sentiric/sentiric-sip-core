...
impl SdpBuilder {
    ...
    pub fn with_ptime(mut self, ptime: u8) -> Self { self.ptime = ptime; self }
    pub fn with_rtcp(mut self, enabled: bool) -> Self { self.enable_rtcp_attribute = enabled; self }

    pub fn add_codec(mut self, id: u8, name: &str, rate: u32, params: Option<&str>) -> Self {
        self.codecs.push(Codec { id, name: name.to_string(), rate, params: params.map(|s| s.to_string()) });
        self
    }
    ...
    pub fn build(self) -> String {
        ...
        if self.enable_rtcp_attribute { let _ = write!(sdp, "a=rtcp:{} IN IP4 {}\r\n", self.port + 1, self.ip_address); }
        for codec in &self.codecs {
            let _ = write!(sdp, "a=rtpmap:{} {}/{}\r\n", codec.id, codec.name, codec.rate);
            if let Some(params) = &codec.params { let _ = write!(sdp, "a=fmtp:{} {}\r\n", codec.id, params); }
        }
