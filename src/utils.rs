...
pub fn extract_username_from_uri(uri: &str) -> String {
    ...
    let pure_user = if let Some(idx) = user_part.find(';') {
        &user_part[..idx]
    } else {
        user_part
    };
...
pub fn extract_socket_addr(uri: &str) -> Option<SocketAddr> {
    ...
    let host_port_part = if let Some(semi_idx) = s.find(';') {
        &s[..semi_idx]
    } else {
        s
    };
    let host_port = if let Some(at_idx) = host_port_part.find('@') {
        &host_port_part[at_idx + 1..]
    } else {
        host_port_part
    };
...
pub fn apply_topology_hiding(packet: &mut SipPacket, public_ip: &str, public_port: u16) -> bool {
    let old_contact_val = match packet.get_header_value(HeaderName::Contact) {
        ...
        None => {
            ...
            packet
                .headers
                .push(Header::new(HeaderName::Contact, new_contact));
            return true;
        }
    };
